...
function AdminClients() {
  ...
  const loadClientsWithDetails = async () => {
    try {
      ...
      const clientsData = await adminApi.get('/clients');
      ...
      const detailsPromises = clientsList.map(async (client) => {
        try {
          const detailData = await adminApi.get(`/clients/${client.id}`);
          return {
            id: client.id,
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Immeubles</th>
                        ...
                        <tr>
                          <td colSpan="6" className="px-6 py-4 bg-gray-50">
                            <div className="space-y-4">
                              ...
                              {/* Immeubles */}
                              {details?.immeubles ? (
                                details.immeubles.length > 0 ? (
                                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                    {details.immeubles.map((immeuble) => (
                                      <div 
                                        key={immeuble.id} 
