...
export default function CompteurTypeGuide({ selectedType, onTypeChange, existingPrincipal = false }) {
  const types = [
    {
      value: 'principal',
      ...
      situation: '1 seul compteur pour tout l\'immeuble',
      disabled: existingPrincipal,
      disabledReason: 'Un compteur principal existe déjà',
      svg: (
      ...
      avantages: ['Simple à gérer', 'Pas de sous-compteurs', 'À créer en premier'],
      suited: 'Première étape d\'un système divisionnaire'
    },
    {
      value: 'divisionnaire',
      ...
      situation: 'Compteur individuel par appartement (après le principal)',
      svg: (
        <svg viewBox="0 0 300 200" className="w-full h-auto">
  ...
  const colorScheme = selectedTypeInfo ? COLORS[selectedTypeInfo.color] : null;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {types.map((type) => {
          ...
          const colors = COLORS[type.color];
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <div className={`w-3 h-3 rounded-full mt-1 flex-shrink-0 ${isSelected ? colors.bgDark : 'bg-gray-300'}`} />
                <div className="flex-1">
                  ...
                  {isDisabled && (
                    <p className="text-xs text-red-600 mt-2 font-medium">⚠️ {type.disabledReason}</p>
                  )}
                </div>
              </div>
            </button>
      ...
      {/* Schéma détaillé */}
      {selectedTypeInfo && colorScheme && (
        <div className={`${colorScheme.bg} border ${colorScheme.borderLight} rounded-lg p-6`}>
          <div className="flex items-start gap-3 mb-4">
            <Info className={`h-5 w-5 ${colorScheme.text} flex-shrink-0 mt-0.5`} />
            <div>
              <h4 className={`font-medium ${colorScheme.textDark} mb-1`}>
          ...
          <div className="space-y-2">
            <p className={`text-sm font-medium ${colorScheme.textDark}`}>
              ✓ Avantages :
            </p>
            <ul className="space-y-1">
              {selectedTypeInfo.avantages.map((avantage, index) => (
                <li key={index} className={`text-sm ${colorScheme.text} flex items-start`}>
          ...
          <div className={`mt-4 pt-4 border-t ${colorScheme.borderLight}`}>
