...
/**
 * ENDPOINT TEMPORAIRE : Nettoyer les immeubles archivés
 * À appeler via reqbin.com puis SUPPRIMER après utilisation
 * 
 * POST https://ton-app.railway.app/api/v1/admin/migrate/delete-archives
 */
router.post('/migrate/delete-archives', async (req, res) => {
  const client = await pool.connect();
  
  try {
    await client.query('BEGIN');

    // Étape 1 : Compter combien d'immeubles seront supprimés
    const countResult = await client.query(
      'SELECT COUNT(*) as count FROM immeubles WHERE archived_at IS NOT NULL'
    );
    const archivedCount = parseInt(countResult.rows[0].count);

    if (archivedCount === 0) {
      await client.query('ROLLBACK');
      return res.json({
        success: true,
        message: 'Aucun immeuble archivé à supprimer',
        deleted: 0
      });
    }

    // Étape 2 : Supprimer les données liées en cascade
    const deletedCompteurs = await client.query(
      `DELETE FROM compteurs_eau 
       WHERE immeuble_id IN (SELECT id FROM immeubles WHERE archived_at IS NOT NULL)`
    );

    const deletedDecomptes = await client.query(
      `DELETE FROM decomptes_eau 
       WHERE immeuble_id IN (SELECT id FROM immeubles WHERE archived_at IS NOT NULL)`
    );

    const deletedLocataires = await client.query(
      `DELETE FROM locataires 
       WHERE immeuble_id IN (SELECT id FROM immeubles WHERE archived_at IS NOT NULL)`
    );

    const deletedProprietaires = await client.query(
      `DELETE FROM proprietaires 
       WHERE immeuble_id IN (SELECT id FROM immeubles WHERE archived_at IS NOT NULL)`
    );

    // Étape 3 : Supprimer les immeubles archivés
    const deletedImmeubles = await client.query(
      'DELETE FROM immeubles WHERE archived_at IS NOT NULL'
    );

    // Étape 4 : Recalculer les unités pour tous les utilisateurs
    await client.query(`
      UPDATE subscriptions 
      SET total_units = (
        SELECT COALESCE(SUM(nombre_appartements), 0) 
        FROM immeubles 
    ...
    res.json({
      ...
      deleted: {
        immeubles: archivedCount,
        compteurs_eau: deletedCompteurs.rowCount,
        decomptes_eau: deletedDecomptes.rowCount,
        locataires: deletedLocataires.rowCount,
        proprietaires: deletedProprietaires.rowCount
      },
      note: 'SUPPRIMEZ CET ENDPOINT MAINTENANT !'
    });
  } catch (error) {
    await client.query('ROLLBACK');
