-- ===================================
-- COPRO MANAGER - SCHEMA TRANSACTIONS
-- ===================================
...
CREATE TABLE IF NOT EXISTS fournisseurs (
  ...
  type VARCHAR(50), -- eau, chauffage, electricite, ascenseur, nettoyage, entretien, syndic, assurance, autre
  email VARCHAR(255),
  telephone VARCHAR(20),
  iban VARCHAR(50),
  numero_tva VARCHAR(50),
  tags TEXT[], -- Pour détection auto (ex: ["VIVAQUA", "EAU"])
...
CREATE TABLE IF NOT EXISTS factures (
  ...
  statut VARCHAR(20) DEFAULT 'pending', -- pending, paid, partial, overdue
  mode_repartition VARCHAR(20) DEFAULT 'auto', -- auto (millièmes), manuel, unique (un seul proprio)
  description TEXT,
  fichier_url TEXT, -- URL du fichier PDF (optionnel)
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Table: factures_repartition (détail de la répartition par propriétaire)
...
-- Table: transactions (historique comptable)
CREATE TABLE IF NOT EXISTS transactions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  immeuble_id UUID REFERENCES immeubles(id) ON DELETE CASCADE,
  facture_id UUID REFERENCES factures(id) ON DELETE SET NULL,
  date_transaction DATE NOT NULL,
  type VARCHAR(50) NOT NULL, -- charge, versement, remboursement, autre
  montant DECIMAL(10,2) NOT NULL,
  description TEXT,
  reference VARCHAR(100), -- Numéro de référence (facture, virement, etc.)
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Table: transactions_proprietaires (lien transaction <-> propriétaire pour les charges)
